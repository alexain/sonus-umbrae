...
export class TimingEditor {
  ...
  mount(): HTMLElement {
    ...
    const chanceRow=document.createElement('div');chanceRow.className='object-builder-chance-row';const chance=document.createElement('input');chance.type='range';chance.min='0';chance.max='100';chance.step='1';chance.value=this.state.value.match(/\bchance\s+(\d+(?:\.\d+)?)/i)?.[1]??'100';const chanceOut=document.createElement('output');chanceOut.textContent=`${chance.value}%`;chance.addEventListener('input',()=>chanceOut.textContent=`${chance.value}%`);const chanceWrap=document.createElement('div');chanceWrap.className='object-builder-mini-slider';chanceWrap.append(chance,chanceOut);const looseLabel=document.createElement('label');looseLabel.className='object-builder-clock-extra-toggle';const loose=document.createElement('input');loose.type='checkbox';loose.checked=/(?:^|[,\s])loose(?:,|$)/i.test(this.state.value);looseLabel.append(loose,document.createTextNode(' Loose'));const probability=document.createElement('div');probability.className='object-builder-timing-probability';const probabilityHead=document.createElement('div');probabilityHead.className='object-builder-timing-probability-head';probabilityHead.innerHTML='<strong>EVENT PROBABILITY</strong><span>Chance is evaluated independently for each scheduled event.</span>';probability.append(probabilityHead);chanceRow.append(field('Chance',chanceWrap),looseLabel);probability.append(chanceRow);
