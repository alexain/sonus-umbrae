...
Sonus Umbrae includes editor-only snippets for expanding common object skeletons quickly. A snippet begins with `@` and is expanded with `Tab`; the `@...` form is never part of the Sonus language seen by the compiler. For example:
...
See the complete [editor snippets reference](./SNIPPETS.md) for commands, aliases, supported templates, expansion examples, and `.full` behavior.
